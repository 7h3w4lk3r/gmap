...
def details(request, address):
	...
	for ik in o['host']:
		...
		if str(saddress) == address:
			...
			if re.search('^gmapsched\_[0-9\.]+', request.session['scanfile']):
				m = re.search('^gmapsched\_[0-9\.]+\_(.+)', request.session['scanfile'])
...
def index(request, filterservice="", filterportid=""):
	...
	r['gmapver'] = 'gmap '+gitcmd.read()+'<br>This project is currently a beta, please <b>DO NOT</b> expose gmap to internet.<br>This version is <b>NOT</b> production ready.'
	...
	if 'scanfile' in request.session:
		...
		for i in xmlfiles:
			...
			filename = i
			if re.search('^gmapsched\_[0-9\.]+',i):
				m = re.search('^gmapsched\_([0-9\.]+)\_(.+)',i)
