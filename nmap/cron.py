...
for i in schedfiles:
	if re.search('^[a-f0-9]{32,32}\.json$', i.strip()) is not None:
		...
		if nextrun <= time.time():
			...
			'sleep 5 && mv /tmp/'+str(sched['number'])+'_'+sched['params']['filename']+'.active /opt/xml/gmapsched_'+str(sched['lastrun'])+'_'+sched['params']['filename']+' && '+
			'ls -lart /opt/xml/gmapsched_'+str(sched['lastrun'])+'_'+sched['params']['filename']+' && python3 '+cdir+'/cve.py gmapsched_'+str(sched['lastrun'])+'_'+sched['params']['filename']+'').readlines()
